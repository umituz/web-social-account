/**
 * Threads OAuth Service Implementation
 */
...
export class ThreadsOAuthService implements IOAuthService {
  ...
  constructor(config: PlatformConfig) {
    ...
    if (!this.config.validate()) {
      throw new ConfigurationError("threads", "Invalid Threads OAuth configuration");
    }
  }
  ...
  async exchangeCodeForToken(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new OAuthError("threads", error?.error?.message || "Token exchange failed", error);
      }
      ...
      throw new NetworkError("threads", "Failed to exchange code for token", error);
    }
  }
  ...
  async refreshToken(
    ...
    try {
      ...
      if (!response.ok) {
        throw new InvalidTokenError("threads", { refreshToken });
      }
      ...
      throw new NetworkError("threads", "Failed to refresh token", error);
    }
  }

  async revokeToken(platform: string, token: string): Promise<void> {
    try {
      await fetch(`https://graph.threads.net/v1.0/me/permissions?access_token=${token}`, {
        method: "DELETE",
      });
    } catch (error) {
      throw new NetworkError("threads", "Failed to revoke token", error);
    }
  }
